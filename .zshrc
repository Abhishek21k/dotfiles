...
function geoip() {
    if [ -z "$1" ]; then
        echo "Usage: geoip <domain>"
        return 1
    fi
    dig "$1" +short | while read ip; do
        echo -e "\nIP: $ip"
        curl -s ipinfo.io/$ip
    done
}
...
if [ -f '/Users/zenitsu/Downloads/google-cloud-sdk/completion.zsh.inc' ]; then . '/Users/zenitsu/Downloads/google-cloud-sdk/completion.zsh.inc'; fi
...
# bun completions
[ -s "/Users/zenitsu/.bun/_bun" ] && source "/Users/zenitsu/.bun/_bun"
...
_hermes() {
    ...
    _arguments         '-p[Profile name]:profile:($profiles)'         '--profile[Profile name]:profile:($profiles)'         '1:command:(chat model gateway setup status cron doctor config skills tools mcp sessions profile update version)'         '*::arg:->args'

    case $words[1] in
        profile)
            _arguments '1:action:(list use create delete show alias rename export import)'                         '2:profile:($profiles)'
            ;;
    esac
}
